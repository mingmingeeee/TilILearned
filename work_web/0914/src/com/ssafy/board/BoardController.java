...
public class BoardController {
	...
	private void readMenu() {
		...
		try {
			...
			if (line.equals("exit")) {
				...
				// idx + 1 이후값 가져오기 -> substring(idx + 1)
				// args: [new title, content, writer]
				execute(cmd, args);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void execute(String cmd, String[] args) {
		...
		case "list":
			try {
				...
				for (Board b : list) {
					...
					sb.append(b.getNo()).append("\t").append(b.getTitle()).append("\t").append(b.getId()).append("\t")
		...
		case "detail":
			try {
				if (args != null && args.length == 1) { // args: 글 번호 (no)
		...
		case "modify":
			try {
				...
				System.out.println("새로운 글을 수정하지 못했습니다.");
				e.printStackTrace();
			}
			break;
