...
select * from user where substring(name, 1, 1) like '토';
...
select * from board order by date_format(createtime, '%Y') desc, num asc;

#11. user 에서 id의 세번째 글자가 ‘o’ 인 data를 출력하시오.
select * from user where substring(id, 3, 1) like 'o';

#12. board 에서 content의 공백을 모두 ‘+’ 로 치환해서 출력하시오.
select replace(content, ' ', '+') '치환' from board; 

#13. board 에서 writer를 중복없이 출력하시오.
select distinct(writername) writername from board;