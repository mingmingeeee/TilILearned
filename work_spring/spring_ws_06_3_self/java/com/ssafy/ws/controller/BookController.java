...
public class BookController {
	...
	@GetMapping("/view")
	public String view(@RequestParam("isbn") String isbn, Model model) {
	...
	@GetMapping("/modify")
	public String modify(@RequestParam("isbn") String isbn, Model model) {
		Book book = bookService.select(isbn);
		model.addAttribute("book", book);
		return "modify";
	}
	...
	@PostMapping("/modify")
	public String modify_result(Book book, RedirectAttributes redirectAttributes) {
	...
	public String doRegist(@ModelAttribute Book book/* , Model model */,
			@RequestParam(value = "upfile") MultipartFile file, Model model) throws Exception {

		// 클라이언트로 부터 전달받은 파일이 존재하면 아래와 같이 처리 => null이 아니고 0 이상
		if (file != null && file.getSize() > 0) {

			// 파일을 저장할 폴더 지정
			// C:\SSAFY\bug_8th_4_minjeong\work_spring\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\spring_ws_05_3
			// 아래 생성됨
