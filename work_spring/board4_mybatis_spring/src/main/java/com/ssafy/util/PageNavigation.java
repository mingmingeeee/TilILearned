...
public class PageNavigation {
	...
	private boolean startRange; // 이전 버튼 활성화 여부 => 시작 페이지인지?
	private boolean endRange; // 다음 버튼 활성화 여부 => 마지막 페이지인지?
	private int totalCount; // DB에서 구해와라~
	private int newCount; // X
	private int totalPageCount; // (tA - 1)/20 + 1 => totalArticle
	private int currentPage; // pgno
	...
	public void makeNavigator() {
		...
		if(totalPageCount < endPage) // ​size만큼 다 안채워졌을 때
