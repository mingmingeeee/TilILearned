...
function makeAptList(data) {
	...
	apts.forEach((apt) => {
		...
		kakao.maps.event.addListener(
				marker,
				"click",
				makeClickListener(map, marker, infowindow, apt["apartmentName"],"apartmentName")
		);
...
function makeAptSaleList(data, pos,type) {
	...
	apts.forEach((apt) => {
		if (apt[type] == click_apt_name) {
			...
			kakao.maps.event.addListener(
					marker,
					"click",
					makeClickListener(map, marker, infowindow, apt[type],type)
			);
			// infowindow.open(map, marker);
...
function makeClickListener(map, marker, infowindow, aptname,type) {
	return function () {
		...
		infowindow.close();
		makeAptSaleList(saleinfo, pos,type);
	};
}
